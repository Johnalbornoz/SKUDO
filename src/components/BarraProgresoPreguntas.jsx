...
export default function BarraProgresoPreguntas({ diagnosticoId, refreshKey }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-indigo-50 border border-indigo-200 w-full shadow-sm">
      <FileCheck className="w-4 h-4 text-indigo-600 shrink-0" />
      <span className="text-sm font-semibold text-slate-800 shrink-0">
        Preguntas con evidencia:
      </span>
      <span className="text-sm font-bold text-indigo-700 shrink-0 tabular-nums">
        {calificadas_con_evidencia} / {total}
      </span>
      <div className="flex-1 min-w-[80px] h-2.5 bg-slate-200 rounded-full overflow-hidden">
        <div
          className="h-full bg-indigo-500 rounded-full transition-all duration-300"
          style={{ width: `${pct}%` }}
        />
      </div>
      <span className="text-sm font-bold text-indigo-700 shrink-0 tabular-nums w-8 text-right">{pct}%</span>
    </div>
  );
}
